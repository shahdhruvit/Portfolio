  ...
  const btn=useMatches({
    ...
    return <div className="w-[32%] lg-mx:w-[46%] md-mx:w-[48%] sm-mx:w-[90%] xs-mx:w-full" data-aos="fade-up" data-aos-duration="800">
        <Card onClick={open} className="!bg-bgColor cursor-pointer transition-transform
         duration-300 ease-in-out mb-5 hover:!scale-[1.02] hover:!shadow-[0_0_10px_1px_#64FFDA80]
        ...
        <Group justify="space-between" mt="xs" mb="xs">
          <Text className="!text-2xl gap-2 !font-bold !text-white sm-mx:!text-xl flex
           items-center" >{props.title} {props.live===true &&<Badge className="!px-1"
            variant="outline" color="red" rightSection={<Indicator className="!mr-0.5" color="red"
             position="middle-end" size={7} processing></Indicator>}>Live</Badge>}</Text>
        ...
        <Text lineClamp={5} className="!text-justify !text-sm xs-mx:text-xs" size="sm" c="dimmed">
         {props.desc}
        </Text>
  
        <Button onClick={open} className="" color="#64FFDA"
         variant="outline" mt="md" radius="md">
          Show More 
        </Button>
      </Card>
      <FullProjectModal opened={opened} close={close} title={props.title}
       desc={props.desc} image={props.image} github={props.github} technologies={props.technologies}/>
      </div>
}
